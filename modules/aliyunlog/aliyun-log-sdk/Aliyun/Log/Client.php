...
class Aliyun_Log_Client {
     ...
     * @throws Aliyun_Log_Exception
     * @return Aliyun_Log_Models_DeleteLogstoreResponse
     */
    ...
    /**
     * 查询已经创建的项目
     * @param Aliyun_Log_Models_ListProjectRequest $request
     */
    public function listProject(Aliyun_Log_Models_ListProjectRequest $request){
        $params = array();
        $headers = array();
        if($request->getOffset()!==null)$params['offset'] = $request->getOffset();
        if($request->getSize()!==null)$params['size'] = $request->getSize();
        ...
        $requestId = isset ( $header ['x-log-requestid'] ) ? $header ['x-log-requestid'] : '';
        $resp = $this->parseToJson ( $resp, $requestId );
        return new Aliyun_Log_Models_ListProjectResponse($resp,$header);
    }
