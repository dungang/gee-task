...
-- 导出  表 newgeetask.auth_item_child 结构
...
-- 导出  表 newgeetask.gt_change_category 结构
...
-- 导出  表 newgeetask.gt_event_handler 结构
...
-- 导出  表 newgeetask.gt_project_member 结构
...
-- 导出  表 newgeetask.gt_project_robot 结构
...
-- 导出  表 newgeetask.gt_robot_message 结构
CREATE TABLE IF NOT EXISTS `gt_robot_message` (
  ...
  `message` varchar(255) DEFAULT NULL COMMENT '消息模板',
  PRIMARY KEY (`id`)
...
INSERT INTO `gt_story` (`id`, `sprint_id`, `story_type`, `status`, `important`, `points`, `project_id`, `user_id`, `last_user_id`, `creator_id`, `created_at`, `updated_at`, `name`, `project_version`, `is_del`) VALUES
	...
	(405, 36, 'requirement', 11, 100, 1, 1, 14, 3, 1, 1540180739, 1540521534, '售卖房型标签区分直营与供应商需求', NULL, 0),
	(406, 37, 'requirement', 11, 100, 1, 1, 3, 3, 1, 1540809094, 1542010522, '扫码登录开发', NULL, 0);
/*!40000 ALTER TABLE `gt_story` ENABLE KEYS */;

-- 导出  表 newgeetask.gt_story_acceptance 结构
...
-- 导出  表 newgeetask.gt_story_active 结构
...
-- 导出  表 newgeetask.gt_story_status 结构
...
-- 导出  表 newgeetask.gt_timeline 结构
